...
xMap<K,V>& xMap<K,V>::operator=(const xMap<K,V>& map){
    //YOUR CODE IS HERE
    if(this!=&map){removeInternalData();
 copyMapFrom(map);
  this->deleteKeys=nullptr;
    this-> deleteValues=nullptr;
}
...
for (auto it = bucket.begin(); it != bucket.end(); ++it){
  Entry* r = *it;
            ...
            if (this->keyEQ(r->key, key)) {
                V retValue = r->value; 
                  if (deleteKeyInMap) {
                    deleteKeyInMap(r->key);
                }
              if (bucket.removeItem(r, deleteEntry))count--;
...
bool xMap<K,V>::remove(K key, V value, void (*deleteKeyInMap)(K), void (*deleteValueInMap)(V)){
 ...
 DLinkedList<Entry*>& bucket = table[index];
       for (auto it = bucket.begin(); it != bucket.end(); ++it){
        Entry* re = *it;
            if (this->keyEQ(re->key, key) && this->valueEQ(re->value, value)) {
                if (deleteKeyInMap) {
                    deleteKeyInMap(re->key);
                }
                if (deleteValueInMap) {
                    deleteValueInMap(re->value);
                }
                if(bucket.removeItem(re, deleteEntry))count--;
                return true;
...
bool xMap<K,V>::containsValue(V value){
   ...
   for (int i = 0; i < capacity; i++) {
       ...
       for (auto it = bucket.begin(); it != bucket.end(); ++it){
        Entry* co = *it;
            if (this->valueEQ(co->value, value)) {
                return true; 
            }
        }
...
void xMap<K,V>::clear(){
  ...
  for (int idx = 0; idx < this->capacity; idx++) {
    ...
    for (auto pEntry : list) deleteEntry(pEntry);
  }
